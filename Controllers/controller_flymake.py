...
from ryu.controller.handle import CONFIG_DISPATCHER, MAIN_DISPATCHER
...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def get_malicious_path(self, start, end):
        ...
        if(start in self.topology and end in self.topology):
            paths = list(nx.all_simple_paths(self.topology, start, end))
            for path in paths:
                if(mid_node in path):
                    print "Diverting through 1", path
                    return path
